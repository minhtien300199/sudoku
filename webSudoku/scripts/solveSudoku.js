...
var isOK = function (i, j, x) //kt ma trận.
{
  ...
  //kiem tra hang thu i da co cai nao trung chua
  for (k = 0; k < max; k++)
    if (solveSudokuArr[i][k] === x) return 0;
  //kiem tra cot thu j da co cai nao trung chua
  for (k = 0; k < max; k++)
    if (solveSudokuArr[k][j] === x) return 0;
  //kiem tra trong o 3x3
  tmpX = i % 3;
  tmpY = j % 3;
  for (k = i - tmpX; k <= i - tmpX + 2; k++)
    for (t = j - tmpY; t <= j - tmpY + 2; t++)
      if (solveSudokuArr[k][t] === x) return 0;
  return 1;
};
var checkCell = function (i, j, x) //kt ma trận.
{
  ...
  //kiem tra hang thu i da co cai nao trung chua
  for (k = 0; k < max; k++)
    if (sudokuArray[i][k] === x) return 0;
  //kiem tra cot thu j da co cai nao trung chua
  for (k = 0; k < max; k++)
    if (sudokuArray[k][j] === x) return 0;
  //kiem tra trong o 3x3
  tmpX = i % 3;
  tmpY = j % 3;
  for (k = i - tmpX; k <= i - tmpX + 2; k++)
    for (t = j - tmpY; t <= j - tmpY + 2; t++)
      if (sudokuArray[k][t] === x) return 0;
  return 1;
};
